...
from .resources.products import blp as ProductBp
from .resources.health import health_bp as HealthBp
...
def create_app(db_url=None):
    ...
    product_service.config["CORS_AUTOMATIC_OPTIONS"] = True
    product_service.config["PROPAGATE_EXCEPTIONS"] = True
    product_service.config["API_TITLE"] = "Product service API"
    product_service.config["API_VERSION"] = "v1"
    product_service.config["OPENAPI_VERSION"] = "3.0.3"
    product_service.config["OPENAPI_URL_PREFIX"] = "/"
    product_service.config["OPENAPI_SWAGGER_UI_PATH"] = "/swagger-ui"
    product_service.config["OPENAPI_SWAGGER_UI_URL"] = "https://cdn.jsdelivr.net/npm/swagger-ui-dist/"
    product_service.config['TESTING'] = True  # Enable testing mode for the Flask app
    ...
    product_service.config["SQLALCHEMY_DATABASE_URI"] = SQLALCHEMY_DATABASE_URI
    db.init_app(product_service)  # db is SQLAlchemy extension

    with product_service.app_context():
        db.create_all()

    api = Api(product_service)
    ...
    product_service.config["JWT_SECRET_KEY"] = os.environ.get("JWT_SECRET_KEY")
    product_service.config["JWT_TOKEN_LOCATION"] = ["headers"]
    JWTManager(product_service)

    @product_service.route("/health")
    def health():
        return jsonify({"status": "healthy"}), 200

    api.register_blueprint(HealthBp)
